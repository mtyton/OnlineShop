...
class CategoryModelTestCase(ExtendedTestCase):
    ...
    def test_bottom_level_categories_success(self):
        top_level_category = Category.objects.filter(
    ...
    def test_bottom_level_categories_for_bottom_level_category(self):
        ...
        self.assertEqual(
            category.bottom_level_categories.first().pk, category.pk
        )
        ...
        for i in range(5):
            expected_categories.append(
                Category.objects.create(
                    category_name=f"LonelyCategory{i}",
                    parent_category=mid_level_category
                )
            )
        bottom_level_categories = top_level_category.bottom_level_categories
        self.assertEqual(
            bottom_level_categories.count(), len(expected_categories)
        )
        for category in expected_categories:
            self.assertTrue(
                bottom_level_categories.filter(pk=category.pk).exists()
            )
